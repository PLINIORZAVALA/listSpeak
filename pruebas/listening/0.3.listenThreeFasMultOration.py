...
def guardar_audio(texto_ingles):
    ...
    try:
        if os.path.exists(ruta_completa_audio):
            cerrar_audio()  # Asegúrate de que el audio se cierra antes de eliminar
            os.remove(ruta_completa_audio)  # Eliminar el archivo existente
...
def cerrar_audio():
    """Cierra el audio en reproducción si está activo."""
    try:
        pygame.mixer.music.stop()  # Detener la música si está reproduciendo
        pygame.mixer.music.unload()  # Cerrar el archivo de audio cargado
    except Exception as e:
        messagebox.showerror("Error al Cerrar Audio", f"No se pudo cerrar el audio: {e}")

def reproducir_audio():
    """Reproduce el archivo de audio generado."""
    try:
        if os.path.exists(ruta_completa_audio):
            cerrar_audio()  # Asegúrate de cerrar el audio antes de cargar uno nuevo
...
def regresar_inicio(ventana_actual):
    ...
    cerrar_audio()  # Cierra el audio antes de regresar
    ventana_actual.destroy()  # Cerrar ventana actual
